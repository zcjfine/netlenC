...
//        error(1, 0, "usage: tcpclient <IPaddress>");
    ...
    serverAddr.sin_addr.s_addr = htonl(0x7f000001);   //0x7f000001 127.0.0.1的16进制 htonl()用于把数字转为网络地址
//    inet_pton(AF_INET, argv[1], &serverAddr.sin_addr); //inet_pton() 用于把 字符串值转换为网络地址
    ...
    Connect(socketfd, connsockaddr, socklen);
...
//    }
    ...
    while (remaining) { //如果还有没法送完的长度
        ...
        int n_written =  Write(socketfd, chars, remaining);
